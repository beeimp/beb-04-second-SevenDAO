...
export default async (req, res) => {
    ...
    try {
        ...
        if (fromUserCtoken) {
            ...
            if (residue > 0) {
                console.log('lastTrxRes');
                let dbres;
                const isSendTokenOk = await sendToken(fromUserCtoken, contractOwnerPubKey, fromUser[0].privatekey);
                if(isSendTokenOk) { dbres = await myClient.db(dbName).collection(collectionName).updateOne({ username: fromUsername }, { $inc: { token: -residue }, }, { upsert: true }); }
                if (dbres) {
                    res.send({ message: 'transaction success!' });
                    return;
                }
                else { res.send({message : "transaction failed on db"}) }
            }
            else {
